...
```{example exemptemp}
Vamos supor que desejamos medir a temperatura máxima do ar, de um local,
...
```{r figts1,echo=FALSE,fig.align='center',fig.cap="Temperaturas médias diárias nas cidades de Manaus (MA) e Porto Alegre (POA)", out.width='90%', fig.asp=.75,message=FALSE}
#library(readxl)
#library(tidyr)
#library(dplyr)
#library(ggplot2)
# Dados baixados em 23/06/2020 de
# http://sisdagro.inmet.gov.br/sisdagro/app/monitoramento/bhs

TempPortoAlegreA <- read_excel("Dados/TempPortoAlegreA.xls")
TempManaus <- read_excel("Dados/TempManaus.xls")
DATA=seq(as.Date("2011-01-02"), as.Date("2020-06-23"), by="days")
Temp=data.frame(DATA=DATA,POA=TempPortoAlegreA$TEMPERATURA,
              MANAUS=TempManaus$TEMPERATURA)

df <- Temp[(length(Temp$DATA)-365):length(Temp$DATA),] %>%
  select(DATA,POA,MANAUS) %>%
  gather(key = "Temperatura", value = "valor", -DATA)

ggplot(df, aes(x = DATA, y = valor)) + 
  geom_line(aes(color = Temperatura), size = 0.5) +
  scale_color_manual(values = c("#1B9E77", "#D95F02")) +
theme_minimal()+labs( x="", y="")+
  annotate(geom="text", y=27,x=as.Date("2020-07-30"), col="#1B9E77", 
           label="Z_1", parse=T,size =5)+
  annotate(geom="text", y=20,x=as.Date("2020-07-30"), col="#D95F02", 
           label="Z_2", parse=T,size =5)
```
...
```{theorem, teogm, name="Teorema de Gauss-Markov"}
...
```{r figpi, fig.align='center',fig.cap="Produção física industrial de Bens Intermediários", out.width='85%', fig.asp=.5,message=FALSE}
#library(readxl)
#library(plotly)
...
```{exercise exsr2,name="anpec-2010"}
