...
Naturalmente, quando estamos trabalhando com um processo estocástico, cada variável aleatória que o compõe possui sua própria distribuição, assim como sua própria massa/densidade de probabilidade e sua própria média/variância. Para um processo estocástico $\{Z_t\}_{t\in\mathbb{Z}}$ definimos, para cada $t\in\mathbb{Z}$, a `r colorize("_função média_", "blue")`, $\mu_t$ e a `r colorize("_função variância_", "blue")` $\sigma^2_t$ respectivamente por
...
`r colorize("_função de autocovariância_", "blue")`, abreviada `r colorize("_FACV_", "blue")`, como
...
Analogamente. definimos a `r colorize("_função de autocorrelação_", "blue")`, abreviada `r colorize("_FAC_", "blue")`, por
\begin{equation}
...
```{r stac,name="ST",echo=FALSE,fig.show='hold',out.width=c('48%','48%'),fig.asp=.65,message=FALSE, fig.cap=' Séries estacionárias: (a) Ruído branco, (b) ARMA(1,2).'}
set.seed(1231)
eps=rnorm(200)+30
x=eps+0.1*(1:200)
sk=arima.sim(n=200,model=list(ar=0.6,ma=c(-0.2,0.6)))
plot(eps,type="l",lwd=2,cex.lab=1.4, xlab="(b) tempo",ylab="", main="", cex.main=1.7,col="cadetblue4")
plot(sk+6,type="l",lwd=2,cex.lab=1.4, xlab="(a) tempo",ylab="", main="", cex.main=1.7,col="cadetblue4")
```
...
```{r nsstac,echo=FALSE,fig.show='hold',out.width=c('48%','48%'),fig.asp=.65,message=FALSE, fig.cap=' Séries não-estacionárias apresentando: (a) Tendência linear, (b) quebra estrutural representada pela mudança de nível da série, (c) tendência estocástica e (d) sazonalidade.'}
set.seed(1231)
eps=rnorm(200)+30
x=eps+0.1*(1:200)
plot(x,type="l", lwd=2,cex.lab=1.2, xlab="(a) tempo",ylab="", main="Série temporal com tendência linear", cex.main=1.4,col="cadetblue4")
y= 30*(sin(2*pi*(1:500)/250)^2)+rnorm(500,sd=4)
plot(y, type="l", lwd=2,cex.lab=1.2, xlab="(b) tempo",ylab="", main="Série temporal com sazonalidade", cex.main=1.4,col="cadetblue4")
z=c(rnorm(100)+7,rnorm(100)+10)
plot(z,type="l",lwd=2,cex.lab=1.2, xlab="(c) tempo",ylab="", main="Série temporal com mudança de Nível", cex.main=1.4,col="cadetblue4")
w=cumsum(rnorm(500))
plot(w,type="l",lwd=2,cex.lab=1.2, xlab="(d) tempo",ylab="", main="Série temporal com tendência Estocástica", cex.main=1.4,col="cadetblue4")
...
Analogamente para a função de autocorrelação. Ou seja, num processo fortemente estacionário a covariância entre $Z_t$ e $Z_s$ depende somente da diferença temporal $|t - s|$ e não dos tempos $t$ e $s$. Ou ainda, podemos dizer que a $\mbox{Cov}(Z_t,Z_{t+h})$ depende apenas da distância temporal $h$ entre as variáveis (chamada de `r colorize("_defasagem_", "blue")`  ou `r colorize("_lag_", "blue")` entre as variáveis), e não do tempo $t$. Isto permite simplificar a notação:
...
> `r colorize("Operador defasagem", "blue")` 
\[
 L^j y_t = y_{t-j}
\]
...
Em outras palavras, um ruído branco é uma sequência de variáveis não-correlacionadas com média constante. `r colorize(" Denotaremos um processo ruído branco por RB($0,\\sigma^2$)", "blue")`. Escreveremos ainda $\varepsilon_t\sim RB(0,\sigma_\varepsilon^2)$ para dizer que $\{\varepsilon_t\}_{t}$ é um processo ruído branco com média 0 e variância $\sigma^2_\varepsilon$
...
```{r rbsim,echo=TRUE,fig.show='hold',out.width=c('33%','33%','33%'),fig.asp=.65,message=FALSE, fig.cap=' Ruído branco gaussiano simulado,FAC amostral e FACP  amostral'}
set.seed(5647)
eps=rnorm(200)
...
```{example, name="Média-Móvel de ordem 1"} 
Este é um exemplo simples de um processo estacionário que não é um ruído branco. Suponha que
```
`r colorize(" Processo MA(1)", "blue")`
...
> 
`r colorize(" A média do processo AR(1)", "blue")`  é
\[
...
> 
`r colorize(" A variância do AR(1)", "blue")`  é
\[
...
> 
`r colorize(" função de autocovariância de _lag_", "blue")` $h$ é dada por
\begin{eqnarray*}
...
> 
`r colorize("A função de autocorrelação de _lag_", "blue")` _h_  é  dada por
\begin{equation*}
...
`r colorize(" O Passeio aleatório ", "blue")` pode ser representado por 
\[
...
`r colorize(" A média do passeio aleatório ", "blue")` é
...
e `r colorize(" a variância do passeio aleatório ", "blue")` é
...
A `r colorize(" função de autocorrelação de um passeio aleatório ", "blue")`  é facilmente obtida
\begin{align*}
...
>
`r colorize(" FACV do passeio aleatório", "blue")` 
\[
...
>
`r colorize(" FAC do passeio aleatório", "blue")`  
\[
...
```{r rw,name="ST",echo=TRUE,fig.show='hold',out.width=c('30%','30%','30%'),fig.asp=.65,message=FALSE, fig.cap=' Passeio aleatório simulado (a), FACV amostral (b) e FAC amostral (c).'}
set.seed(1231)
eps=rnorm(200)
rw=cumsum(eps)
...
>
`r colorize(" O processo AR(_p_)", "blue")`  
...
>
`r colorize(" Definição com o operador _lag_", "blue")`  
\begin{align*}
...
```{r ar105,name="ST",echo=TRUE,fig.show='hold',out.width=c('33%','33%','33%'),fig.asp=.65,message=FALSE, fig.cap='AR(1) simulado com coeficiente $\\phi_1=0.5$ (a), FACV amostral (b) e FAC amostral (c).'}
set.seed(12322)
...
```{r arm105,name="ST",echo=TRUE,fig.show='hold',out.width=c('33%','33%','33%'),fig.asp=.65,message=FALSE, fig.cap='AR(1) simulado com coeficiente $\\phi_1=-0.5$ (a), FACV amostral (b) e FAC amostral (c).'}
set.seed(12322)
...
```{r ar108,name="ST",echo=TRUE,fig.show='hold',out.width=c('33%','33%','33%'),fig.asp=.65,message=FALSE, fig.cap='AR(1) simulado com coeficiente $\\phi_1=0.8$ (a), FACV amostral (b) e FAC amostral (c).'}
set.seed(12322)
...
```{r ar2,name="ST",echo=TRUE,fig.show='hold',out.width=c('33%','33%','33%'),fig.asp=.65,message=FALSE, fig.cap='AR(2) simulado com coeficiente $\\phi_1=0.5$ e $\\phi_1=-0.7$  (a), FACV amostral (b) e FAC amostral (c).'}
set.seed(12322)
...
```{r ar3,name="ST",echo=TRUE,fig.show='hold',out.width=c('33%','33%','33%'),fig.asp=.65,message=FALSE, fig.cap='AR(3) simulado com coeficiente $\\phi_1=0.5$, $\\phi_2=-0.7$ e $\\phi_3=0.6$   (a), FACV amostral (b) e FAC amostral (c).'}
set.seed(12322)
...
>
`r colorize(" O processo MA(_q_)", "blue")`  
\[
...
>
`r colorize("A variância do MA(1)", "blue")`  
...
>
`r colorize(" _FACV_ do MA(1)", "blue")` 
\begin{eqnarray*}
...
>
`r colorize(" _FACV_  _FAC_ e do MA(1)", "blue")` 
\[
\gamma(h)=\begin{cases}
...
>
`r colorize(" Média do MA(_q_)", "blue")` 
...
>
`r colorize("A variância do MA(_q_)", "blue")`  
\begin{eqnarray*}
...
>
`r colorize(" _FACV_ do MA(_q_)", "blue")` 
\begin{align*}
  \gamma(h)&=\mbox{Cov}(Y_t,Y_{t-h} )\\
...
>
`r colorize(" _FACV_ e _FAC_ do MA(_q_)", "blue")` 
\[\gamma(h)=\left\{
\begin{array}{cc}
...
```{r ma11,name="ST",echo=TRUE,fig.show='hold',out.width=c('33%','33%','33%'),fig.asp=.65,message=FALSE, fig.cap='MA(1) simulado com coeficiente $\\theta_1=1$, (a) FACV amostral (b) e FAC amostral (c).'}
# MA(1) Simulado
set.seed(12322)
...
```{r ma1m08,name="ST",echo=TRUE,fig.show='hold',out.width=c('33%','33%','33%'),fig.asp=.65,message=FALSE, fig.cap='MA(1) simulado com coeficiente $\\theta_1=-0.8$, (a) FACV amostral (b) e FAC amostral (c).'}
# MA(1) Simulado
set.seed(12322)
...
```{r ma1m0804,name="ST",echo=TRUE,fig.show='hold',out.width=c('33%','33%','33%'),fig.asp=.65,message=FALSE, fig.cap='MA(2) simulado com coeficiente $\\theta_1=-0.8$ e $\\theta_1=0.4$, (a) FACV amostral (b) e FAC amostral (c).'}
# MA(1) Simulado
set.seed(1234)
...
```{r ma1m080414,name="ST",echo=TRUE,fig.show='hold',out.width=c('33%','33%','33%'),fig.asp=.65,message=FALSE, fig.cap='MA(2) simulado com coeficiente $\\theta_1=-0.8$, $\\theta_1=0.4$ e $\\theta_1=1.4$, (a) FACV amostral (b) e FAC amostral (c).'}
# MA(1) Simulado
set.seed(12341)
...
>
`r colorize(" O modelo ARMA(_p_,_q_)", "blue")` 
\begin{equation}
...
```{example, arma23,name=" Modelo ARMA(2,3) "} 
O Modelo ARMA(2,3) é escrito como
...
```{example, causar1, name="Causalidade do processo AR(1)"}
O modelo AR(1):
...
O  conceito de `r colorize(" causalidade ", "blue")` consiste em escrever um processo AR($q$) como um MA($\infty$).

---------------------------------------------------------------------------

> 
Um  processo linear  $\{Y_t \}$ é `r colorize(" CAUSAL ", "blue")` (estritamente, uma função causal de  $\{\varepsilon_t \}$) se existem reais $\psi_0,\psi_1,\cdots$ satisfazendo $\sum_{j=0}^{\infty}|\psi_j|<\infty$ e tais que
\begin{align}
...
> 
Um processo  linear  $\{Y_t \}$  é dito ser `r colorize(" INVERTÍVEL ", "blue")`  (estritamennte, uma função invertível de $\{\varepsilon_t \}$) se existem reais $\mbox{Var}phi_1,\mbox{Var}phi_2,\cdots$ satisfazendo $\sum_{j=0}^{\infty}|\mbox{Var}phi_j|<\infty$ e tais que
\begin{align}
...
```{example, invma1, name="Invertibilidade do processo MA(1)"}
...
```{theorem, soluest}
Uma (única) solução **estacionária** para $\Phi(L)y_t = e_t$ existe se, e  somente, as  raízes de $\Phi(z)$ **não pertence ao círculo de raio um**, ou  seja,
...
Para um modelo geral MA($q$), definimos o `r colorize(" polinômio característico MA ", "blue")` como
\begin{equation*}
...
```{theorem, invert}
Um processo MA($q$) é invertível se, e somente  se, as raízes de $\Theta(z)$ estão fora do círculo
unitário,  isto é,
...
```{theorem, causaarma} 
Se $\Phi(\cdot)$ e  $\Theta(\cdot)$ não possuem fatores em comum, existe
...
```{exercise pest} 
Defina processo estocástico e ilustre graficamente. Explique o que é a realização de um processo estocástico e por que séries econômicas podem ser entendidas como geradas por um processo estocásticos.
```

<br>

```{exercise stce} 
 Seja $\{y_t\}^T_{t=1}$ uma série temporal. Quais características essa série deve apresentar para ser considerada uma série de covariância estacionária?
```
...
```{exercise ma2} 
 Considere um processo MA(2): $y_t=e_t+\alpha_1e_{t-1}+\alpha_2e_{t-2};$ onde $e_t \sim
RB(0,\sigma^2_e)$.
...
```{exercise exarmamet}
Explique os passos que devem ser seguidos para a modelagem de uma série temporal na metodologia ARMA.
```

<br>

```{exercise, a145, name="ANPEC 2014-5"}
Suponha que $Y_t$ seja representado pelo seguinte processo autorregressivo de primeira ordem:
...
```{exercise, a1410, name="ANPEC 2014-10"}
...
```{exercise, a1310, name="ANPEC 2013-10"}
...
```{exercise, a1313, name="ANPEC 2013-13"}
Considere o seguinte processo $x_t = \mu + e_t + \alpha_1 e_{t-1}$, para $t=1,2,\cdots$, no qual $e_t$ é uma sequência i.i.d com média 0 e variância $\sigma_e^2$ .
Julgue as seguintes afirmativas:
...
```{exercise, a128, name="ANPEC 2012-08"}
...
```{exercise, a1111, name="ANPEC 2011-11"}
Julgue as seguintes afirmativas:
...
```{exercise, a0915, name="ANPEC 2009-15"}
...
```{exercise, s200}
Considere uma série temporal com 200 observações. A Figura \@ref(fig:exarma) mostra a evolução da série ao longo do tempo. A tabela a seguir fornece as autocorrelações, $\rho$´s, e autocorrelações parciais, $\phi$s, estimados a partir dessa série.
```

```{r exarma,name="ST",echo=FALSE,fig.show='hold',out.width=c('95%'),fig.asp=.5,message=FALSE, fig.cap='Série temporal simulada'}
# MA(1) Simulado
set.seed(12344)
exarma=arima.sim(n=300,model=list(ar=0.5,ma=c(0.4)))
plot(exarma,type="l",lwd=2,cex.lab=1.4, xlab=" tempo",ylab="", main="", cex.main=1.7,col="cadetblue4")
```
...
```{exercise, exprev200} 
Abaixo (Figura 2) encontram-se os gráficos da FAC e FACP calculados para uma série
$\{y_t\}^{200}_{t=1}$.
``` 

<br>

```{r exarmapf,echo=FALSE,fig.show='hold',out.width=c('45%','45%'),fig.asp=.65,message=FALSE, fig.cap='FAC e FACP de uma série temporal simulada'}
# MA(1) Simulado
set.seed(5541)
exarma22=arima.sim(n=100,model=list(ar=c(-0.3,-0.2),ma=c(-1,-0.7)))
acf(exarma22,type = "correlation",lag.max = 50,cex.main=1.7,col="cadetblue4",ylab="FAC",lwd=4,main="")
acf(exarma22,type = "partial" ,lag.max = 50,cex.main=1.7,col="cadetblue4",ylab="FACP",lwd=4,main="",ylim=c(-0.8,0.8))
```
