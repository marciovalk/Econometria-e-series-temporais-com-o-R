...
(\#eq:eq1STR)
  ...
  ```{example}
  Um exemplo de  modelo estático é a **curva de Phillips estática**, representada por:
    \begin{equation}
  ...
  (\#eq:eqmdd2)
    ...
    (\#eq:eqPLP2)
      ...
      O processo estocástico \[\{(x_{t1}, x_{t2},\ldots, y_t):\,\,\, t = 1, 2,\ldots, n\}\]
      ...
      <br>
        ...
        > **_Suposição  TS.3:_** \emph{(Média condicional zero ou exogeneidade estrita)}. O termo de erro em qualquer dado período é não correlacionado com as variáveis explicativas em todos os períodos de tempo,  ou seja $\E (u_t | X) = 0$, para $ t = 1, 2,\ldots, n$.
      ...
      $t$, da forma
      \begin{equation}
      \E(u_t|x_{1t},\ldots,x_{tk})=\E (u_t | X_t) = 0,
      (\#eq:exoc)
        \end{equation}
        ...
        ```{theorem}
        Sob as Hipóteses **ST.1**, **ST.2** e **ST.3** os estimadores de MQO são não viesados condicionados a $X$ e, portanto, também incondicionalmente:
          \begin{equation}
        \E(\hat{\beta}_j)=\beta_j, \,\,\,j=1,\ldots,k.
        \end{equation}
        ```
        ...
        ```{theorem, tgm, name="Teorema de Gauss-Markov"}
        Sob as Hipóteses **ST.1** a **ST.5** os estimadores de MQO são os melhores estimadores lineares não viesados condicionais a $X$, ou seja, são **_BLUE_**.
        ```
        ...
        ```{theorem}
        Sob as hipóteses **TS.1** a **TS.6**, as hipóteses do modelo linear clássico para séries temporais, os estimadores MQO são normalmente distribuídos, condicional em $X$. Além disso, a estatística $t$ tem uma distribuição $t$, e cada estatística $F$ tem uma distribuição $F$.
        ```
        ...
        Quando trabalhamos com séries temporais é necessário saber reconhecer se estas séries contém uma tendência temporal. Ignorar o fato de que duas séries temporais podem ser correlacionadas somente porque ambas estão apresentando uma mesma tendência ao longo do tempo, em vez de uma relação causal, pode levar a conclusões errôneas e a possibilidade de uma regressão espúria. Vejamos o exemplo de uma série temporal com tendência temporal:
          ...
          ```{r figcn, fig.align='center',fig.cap="Custo médio do m^2 no Brasil a partir do plano real", out.width='65%', fig.asp=.5,message=FALSE}
        #library(readxl)
        #library(ggplot2)
        ...
        \end{equation}
        em que assume-se que $\{e_t\}$ é i.i.d. com $\E(e_t)=0$ e $\var(e_t)=\sigma^2$.
        ...
        **_tendência temporal quadrática_**,
        ...
        (\#eq:tr2)
          ...
          Sazonalidade ocorre quando uma série exibe comportamentos semelhantes em determinados períodos. Um exemplo é a Produção física industrial de Bens Intermediários.
          
          
          
          
          ```{r figpi, fig.align='center',fig.cap="Produção física industrial-Bens Intermediários", out.width='85%', fig.asp=.5,message=FALSE}
          #library(readxl)
          #library(plotly)
          #Fonte: "IBGE - Pesquisa Industrial Mensal - Produção Física"
          
          prodind <- read_excel("Dados/ProducaoIndustrial.xls")
          plot_ly(prodind,x=~Data, y=~intermediarios,mode= "lines",type = "scatter")%>% layout(xaxis = list(title = "", showgrid = F),yaxis = list(title = "", showgrid = F))
          ```
          ...
          (\#eq:saz1)
            ...
            Considere o modelo  \@ref(eq:saz1), para $k = 2$, ou seja 2 regressores. Podemos obter os seus
            estimadores, $\hat{\beta}_1$ e $\hat{\beta}_2$, através do seguinte procedimento:
            ...
            Um processo estocástico é covariância estacionária se $\E (x_t)$ é constante,
            $\var(x_t)$ é constante e para qualquer $t, h\geq 1$, $\cov(x_t,x_{t + h})$ depende apenas em $h$, e não em $t$. Mais adiante abordaremos essa definição com maior profundidade.
            ...
            Se, para um processo de covariância estacionária $\cor(x_t, x_{t + h})\rightarrow 0$ quando $h\rightarrow \infty$, dizemos que este processo de covariância estacionária é fracamente dependente.
            ...
            <br>
              ...
              ```{exercise} 
            Sobre regressão com séries temporais responda:
            ...
            <br>
              ...
              ```{exercise,name="anpec-2010"}
            Considere o modelo de regressão linear múltipla com regressores estocásticos $y_t = \beta_1 x_{1t} + \beta_2 x_{2t} +
              \varepsilon_t$, no qual $\varepsilon_t$ não é autocorrelacionado e tem média e variância condicionais a $x_{1t}$ e $x_{2t}$ iguais a zero e $s^2$, respectivamente. Por simplicidade, suponha que as variáveis são expressas como desvios com relação às respectivas médias.
            
            Responda:
              
              -   a. Se $\beta_2 = 0$ e incluirmos $x_{2t}$ na regressão, o estimador de mínimos quadrados ordinários de $\beta_1$ será viesado?
              
              -   a. Se não conseguirmos observar $x_{1t}$, mas apenas
            $x_{1t}^*=x_{1t}+u_t$, em que $u_t$ é um erro de medida, e se
            substituirmos $x_{1t}$ por  $x_{1t}$ na regressão, o estimador de
            mínimos quadrados ordinários de $\beta_1$ ainda assim será
            consistente?
              
              -   b. Se $x_{2t} = y_{t-1}$ e relaxarmos a hipótese de que os erros $\varepsilon_t$`s não são autocorrelacionados, o estimador de mínimos quadrados ordinários de $\beta_2$ será consistente, porém não será eficiente?
              
              -   c. Seja $c$ uma constante diferente de zero. Defina $\tilde{y}= cy_t$, $\tilde{x}_{1t}= cx_{1t}$ e  $\tilde{x}_{2t} =
              cx_{2t}$. Os estimadores de mínimos quadrados ordinários (MQO) em uma regressão de  $\tilde{y}$ contra $\tilde{x}_{1t}$ e
            $\tilde{x}_{2t}$ coincidem com os estimadores de MQO em uma regressão de $y_t$ contra $x_{1t}$ e $x_{2t}$?
              
              -   d. A variância do estimador de mínimos quadrados ordinários diverge para infinito à medida que a correlação entre $x_{1t}$ e $x_{2t}$ aproxima-se de 1;
            
            -   e. Denote por $\widehat{\varepsilon}_t$ o resíduo da regressão de mínimos quadrados ordinários. A hipótese de que o erro é correlacionado com $x_{1t}$ pode ser testada utilizando a estatística $\frac{1}{T}\sum_{i=1}^{T}x_{1i}\widehat{\varepsilon}_{i}$?
              ```
            ...
            <br>
              ...
              ```{exercise}  
            Considere uma série do PIB brasileiro com início no primeiro trimestre 1996 e fim no segundo bimestre de 2010. Essa série foi decomposta em sua tendência $(t)$ e variáveis dummy para a sazonalidade, em que $S_i = 1$, se a observação pertence ao trimestre $i$ e $S_i = 0$, caso contrário.
            ...
            <br>
              ...
              ```{exercise}  
            Considere o modelo $y_t=\alpha_0+\delta_0z_t+\delta_1z_{t-1}+\delta_2z_{t-2}+u_t$.
            ...
            <br>
              ...
              ```{exercise}
            Considere o seguinte modelo estático $crime_t=\beta_0+\beta_1+pol_t+u_t$, em que $crime_t$ é um índice de criminalidade no período $t$ e $pol_t$ é o número de policiais em $t$.
            ...
            <br>
              ...
              ```{exercise}
            Um modelo de ajustamento parcial é dado por:
              $$y^*_t=\beta_0+\beta_1x_t+e_t$$
              $$y_t-y_{t-1}=\lambda(y^*_t-y_{t-1}),$$
              em que $y^*_t$ é o nível desejável ou ótimo de $y$, e $y_t$ é o nível efetivo (observado). Por exemplo, $y^*_t$ é o
            crescimento desejável nos estoques de uma firma e $x_t$ é o crescimento de vendas da firma. O parâmetro $\lambda$ mede a
            ...
            -   a.  Insira a primeira equação na segunda equação e mostre que podemos escrever $y_t=\alpha_0+\alpha_1 y_{t-1}+\alpha_2 x_t+u_t$. Quem são os $\alpha`s$ em termos dos $\beta`s$ e $\lambda$? Quem é $u_t$ em termos de $e_t$?
              ...
              -   b.  Supondo que $\E(e_t|x_t,y_{t-1})=0$ e todas as séries sejam fracamente dependentes,
            ...
            <br>
              ...
              ```{exercise}
            Imagine o seguinte modelo: $Y_t = \beta_0 + \beta_1 X_t + u_t$ , onde $Y$ é a demanda por moeda,
            $X^*$ é a taxa de juros esperada no longo prazo e $u$ é um termo de erro clássico, não correlacionado com
            $X^*$. Como a variável de expectativa $X^*$ não é diretamente observável, proporemos a seguinte hipótese
            para formação de expectativas (adaptativas): $X^*_t- X^*_{t-1} = \gamma(X_t- X_{t-1} )$, em que $\gamma$, tal que $0 < \gamma < 1$, é conhecido como coeficiente de expectativas.
            ...
            <br>
              ...
              ```{exercise}
            Seja o processo $y_t = e_t + \alpha_1 e_{t-1}$, em que $e_t \sim iid(0,\sigma^2).$
              
              -   a. Calcule $\E(y_t )$, $\var(y_t )$ e $\cov(y_t , y_{t-h} )$, $h = 1, 2, 3,\ldots$. O processo $y_t$ é de covariância estacionária?
            ...
            <br>
              ...
              ```{exercise}
            Seja o processo $y_t = c + \rho y_{t-1} + e_t$, em que $e_t \sim iid(0,\sigma^2)$.
            
            -   a. Qual é a condição de estabilidade para esse processo? Calcule $\E(y_t )$ e $\var(y_t )$ considerando válida a condição de estabilidade.
            
            -   b. Para o processo $y_t$ acima temos que $\cov(y_t,y_{t-h} ) = \frac{\rho^h\sigma^2}{1-\rho^2}$, $h = 1, 2, 3,\ldots$. O processo $y_t$ é de covariância estacionária? Justifique.
